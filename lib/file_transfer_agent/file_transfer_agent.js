...
function file_transfer_agent(context)
{
  ...
  */
  async function upload(largeFileMetas, smallFileMetas)
  {
    ...
    var client = await storageClient.createClient(stageInfo, false);
  ...
  */
  async function transferAccelerateConfig()
  {
    ...
    {
      var client = await SnowflakeRemoteStorageUtil.createClient(stageInfo, false);
  ...
  */
  function getFileNameFromPutCommand(command)
  {
    ...
    var endIndex = command.substring(startIndex).indexOf(' ');
