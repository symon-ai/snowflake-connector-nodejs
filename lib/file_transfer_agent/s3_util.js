...
{
  const { S3Client, GetObjectCommand } = typeof s3 !== "undefined" ? s3 : require('@aws-sdk/client-s3');
  const { Upload } = typeof s3 !== "undefined" ? s3 : require('@aws-sdk/lib-storage');
  ...
  */
  this.getFileHeader = async function (meta, filename)
  {
    ...
    try
    {
      akey = await client.send(new GetObjectCommand(params))
    }
    catch (err)
    {
